...
function updatePhysicalDimensions(){ // only if sizePhys changed
    straightLen=refSizePhys*critAspectRatio;
    mainroadLen=2*straightLen;
}
...
var truck_width=7; 
var motobike_length = 4;
var motobike_width=3
...
motorImg = new Image();
motorImg.src = 'figs/motorcycle.png'
...
function carOrMotobike() {
    return Math.random() >= 0.5 ? "car" : "motobike"
}
function vehTypeRandom(truckFrac) {
    return Math.random() < truckFrac 
            ? "truck" : carOrMotobike()
}

function getVehLength(vehType) {
    return vehType == "car"
            ? car_length : vehType == "truck"
            ? truck_length : motobike_length
}

function getVehWidth(vehType) {
    return vehType == "car"
            ? car_width : vehType == "truck"
            ? truck_width : motobike_width
}

function getVehImg(vehType) {
    return vehType == "car"
            ? carImg : vehType == "truck"
            ? truckImg : motorImg
}

function isCarModel(vehType) {
    return (vehType == "car" || vehType == "motobike")
}
